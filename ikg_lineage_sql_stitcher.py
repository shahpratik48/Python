...
def run_pipeline() -> None:
    ...
    insight_raw = input("Enter insight_type (optional, comma-separated): ").strip()
    profile_table_raw = input("Enter profile_table (optional, comma-separated): ").strip()
    if not insight_raw and not profile_table_raw:
    ...
    override_profile_tables = (
        [value.strip() for value in profile_table_raw.split(",") if value.strip()]
        if profile_table_raw
        else []
    )
