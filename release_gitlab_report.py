...
# Airflow Variable names (do not read at import-time)
POSTGRES_CONN_ID_VAR = "IKG_POSTGRES_CONN_ID"
GITLAB_TOKEN_VAR = "GENESIS_DDLC_IKG_GIT_SECRET"

# These module-level names are intentionally left unset so we don't
# call Airflow Variable.get() at import time. Use the helper getters
# below to retrieve values at runtime depending on environment.
POSTGRES_CONN_ID = None
PRIVATE_TOKEN = None
# Defer reading PROPS / IKG_SCHEMA until runtime so importing this module
# in environments where Airflow Variables are not available does not fail.
PROPS = None
GREENPLUM_SCHEMA = None
...
RECIPIENTS = [
    ...
    "trishul.sangu@ubs.com"
]


# -------------------------------------------------------------------
# Airflow detection and credential helpers
# -------------------------------------------------------------------
try:
    # Try importing Airflow Variable but tolerate environment where Airflow isn't installed
...
def is_running_in_airflow() -> bool:
    ...
    We check both that the airflow package imports and that the
    AIRFLOW_CTX_DAG_ID environment variable exists (set by Airflow at
    runtime). This avoids treating developer machines with the
    airflow package installed as an Airflow run.
    """
...
def get_private_token(allow_prompt: bool = True) -> str:
    ...
    - When running inside Airflow: read from Airflow Variable defined in
      GITLAB_TOKEN_VAR.
    - Locally: prefer environment variable GENESIS_DDLC_IKG_GIT_SECRET,
      otherwise prompt the user if allow_prompt is True (preserves
      existing behavior).
    """
    ...
    # Local fallback: environment variable or prompt
    token = os.environ.get(GITLAB_TOKEN_VAR)
    if token:
        return token
    if allow_prompt:
        return getpass.getpass("Enter your private token: ")
    raise RuntimeError("GitLab token not found in environment and prompt disabled.")


def get_postgres_conn_id() -> Optional[str]:
    """Return the Airflow Postgres connection id when running in Airflow.

    Returns None when running locally; local code paths should continue
    to use GREENPLUM_* constants and prompt for a password as before.
    """
    if is_running_in_airflow():
        return Variable.get(POSTGRES_CONN_ID_VAR)
    return None


def get_greenplum_credentials() -> Optional[dict]:
    """When running in Airflow, resolve the Postgres connection id to credentials.

    Returns a dict with keys: host, port, schema/db, user, password. Returns
    None when not running in Airflow or when resolution fails.
    """
    if not is_running_in_airflow():
        return None

    try:
        # Import locally to avoid import-time dependency
        ...
        return {
            'host': conn.host,
            'port': int(conn.port) if conn.port else GREENPLUM_PORT,
            'db': conn.schema or GREENPLUM_DB,
            'user': conn.login or GREENPLUM_USER,
...
def _send_email_with_attachments(recipients, subject, html_body, attachments, smtp_host: str = 'localhost', smtp_port: int = 25, from_addr: str = 'staat-insights@ubs.com'):
    if not recipients:
        logger.warning("No recipients provided for email, skipping send.")
        return
    # Use Airflow's send_email utility directly. This helper is only
    # invoked when running inside Airflow (see is_running_in_airflow()).
    try:
        from airflow.utils.email import send_email  # type: ignore
        # call send_email with attachments (files parameter)
        send_email(to=recipients, subject=subject, html_content=html_body, files=attachments)
    except Exception as e:
        # Log an explicit error — we do not fall back to smtplib per request
        logger.error(f"Failed to send email via airflow.utils.email.send_email: {e}")


def ensure_greenplum_schema() -> Optional[str]:
    """Ensure GREENPLUM_SCHEMA is populated.

    - When running in Airflow: read from ikg_props.get_airflow_variables().
    - When running locally: default to 'sandbox_prj_smart_insights'.
    Returns the schema name.
    """
    global PROPS, GREENPLUM_SCHEMA
    if GREENPLUM_SCHEMA:
        return GREENPLUM_SCHEMA

    if is_running_in_airflow():
        # Airflow: get schema from props
        try:
            from ikg.scripts.python import props as ikg_props  # type: ignore
            PROPS = ikg_props.get_airflow_variables()
            GREENPLUM_SCHEMA = PROPS.get("IKG_SCHEMA")
            return GREENPLUM_SCHEMA
        except Exception as e:
            logger.warning(f"Could not obtain GREENPLUM_SCHEMA from ikg_props: {e}")
            return GREENPLUM_SCHEMA
    else:
        # Local: use default schema
...
def save_to_greenplum(df, table_name, schema, password, batch_id=None):
    """Save DataFrame to Greenplum database with incremental batch logic and apply grants"""
    ...
    try:
        ...
        # Get current max batch number if not provided
        if batch_id is None:
            try:
            ...
            logger.info(f"Using provided batch number: {current_batch}")
        ...
        # Ensure new columns exist for staat_insight_release
        if table_name == OUTPUT_TABLE1:
            new_columns = ['issue_description', 'issue_summary', 'issue_author', 'issue_author_name', 'uat_comment', 'uat_complete', 'approved_for_release']
            try:
                ...
                raw_conn.commit()
                raw_conn.close()
                logger.info(f"Ensured columns exist on {schema}.{table_name}: {', '.join(new_columns)}")
            except Exception as e:
                logger.warning(f"Could not ensure columns on {schema}.{table_name}: {e}")
        ...
        # Apply owner and grant after write
        try:
            raw_conn = engine.raw_connection()
            with raw_conn.cursor() as cur:
                alter_stmt = sql.SQL("ALTER TABLE {}.{} OWNER TO erd_gpdb_prj_smart_insights").format(
                    sql.Identifier(schema),
                    sql.Identifier(table_name)
                )
                grant_stmt = sql.SQL("GRANT SELECT ON {}.{} TO erd_gpdb_prj_smart_insights_ro").format(
                    sql.Identifier(schema),
                    sql.Identifier(table_name)
                )
            ...
            raw_conn.commit()
            raw_conn.close()
            logger.info(f"Applied owner/grant on {schema}.{table_name}")
        except Exception as e:
            logger.warning(f"Could not apply owner/grant on {schema}.{table_name}: {e}")
...
def main(private_token: Optional[str] = None,
    ...
    # Get credentials
    # private_token and iteration may be provided by caller (e.g. Airflow)
    ...
    for issue in filtered_issues:
        ...
        issue_author_name = ''
        try:
            issue_author = issue.author.get('username') if isinstance(issue.author, dict) else ''
            issue_author_name = issue.author.get('name') if isinstance(issue.author, dict) else ''
        except Exception:
            issue_author = ''
            issue_author_name = ''
        issue_description = ''
        try:
            issue_description = issue.description if hasattr(issue, 'description') and issue.description else ''
        except Exception:
            issue_description = ''
        issue_summary = ''
        data.append({
    ...
    df_issues_branches.to_csv("df_issues_branches.csv", index=False)
    df_merge_requests.to_csv("df_merge_requests.csv", index=False)
    df_issues_branches_merge_requests.to_csv("df_issues_branches_merge_requests.csv", index=False)
    ...
    conditions = [
        ...
        ((df_issues_branches_merge_requests['ikg_branch_name'].str.strip() != '') & (df_issues_branches_merge_requests['state_y'].str.strip() == 'merged')),
        ((df_issues_branches_merge_requests['ikg_branch_name'].str.strip() != '') & (df_issues_branches_merge_requests['state_y'].str.strip() != 'merged'))
    ]
    ...
    for issue_id, group in df_issues_branches_merge_requests.groupby('id_x'):
        ...
        issue_info = {
            ...
            'issue_author': group['issue_author'].iloc[0],
            'issue_author_name': group['issue_author_name'].iloc[0],
            'issue_description': group['issue_description'].iloc[0],
            'issue_summary': '',
        }
    ...
    # Add UAT/approval columns (empty) and force issue summary empty
    df_final['UAT_comment'] = ''
    df_final['UAT_complete'] = ''
    df_final['approved_for_release'] = ''
    df_final['issue_summary'] = ''
    df_final['issue_author_name'] = df_final['issue_author_name'].fillna('')
    
    # Reorder columns
    col_order = [
        'iteration', 'state', 'iteration_end_date', 'title', 'issue_description', 'issue_summary', 'issue_author', 'issue_author_name', 'id_x', 'weight', 'labels', 'epic',
        ...
        'iteration_start_date', 'branch_name', 'UAT_comment', 'UAT_complete', 'approved_for_release', 'comment'
    ]
    ...
    # After we have written and saved the Excel file, send it via email when
    # running inside Airflow. We perform the send here so that `output_file`
    # and `timestamp` are defined and the file exists on disk.
    if is_running_in_airflow():
        try:
            base_url = conf.get('webserver', 'base_url')
            if base_url == Variable.get('AIRFLOW_UAT_URL'):
                subject = f"GitLab Release Report - {run_tag or timestamp} - Dev"
            elif base_url == Variable.get('AIRFLOW_PROD_URL'):
                subject = f"GitLab Release Report - {run_tag or timestamp} - Prod"    
            html_body = (
                "<html><body>"
                "<p>Hi Team,</p>"
                "<p>Release report generated and attached.</p>"
                "<p>Regards,<br/>STAAT Insights Team.</p>"
                "</body></html>"
            )
            _send_email_with_attachments(RECIPIENTS, subject, html_body, [output_file])
            logger.info(f"Sent report email to: {', '.join(RECIPIENTS)}")
        except Exception as e:
            raise RuntimeError(f"Failed to send report email: {e}")
    ...
    # ------------------------------------------------------------------
    # For the "Previous" iteration DAGs that run every Tuesday and
    # Wednesday: we must NOT insert a new batch when the iteration is
    # NOT ending in the current week, because doing so would override
    # any manual UI adjustments made to the latest batch of the previous
    # iteration.
    #
    # Rule:
    #   - If iteration == "Previous" AND the iteration's end date is NOT
    #     in the same ISO calendar week as today → skip DB save and exit.
    #   - For "Current" iteration, or for "Previous" when the iteration
    #     DOES end this week (i.e. it is the release week), proceed
    #     normally and save data.
    #
    # is_iteration_ending_this_week() compares the iteration_end_date
    # ISO week with today's ISO week (year + week number).  It returns
    # True when they match (safe to run) and False when they differ
    # (run should be skipped for Previous iteration).
    if iteration == 'Previous' and not is_iteration_ending_this_week(iteration_end_date):
        logger.info("=" * 60)
        logger.info(
            "⏭️  SKIPPING DB save: iteration='Previous' and the iteration "
            f"(end date={iteration_end_date}) is NOT ending in the current "
            "ISO week. Inserting a new batch now would override manual UI "
            "adjustments made to the previous iteration's latest batch."
        )
        logger.info(
            "   This run will be skipped every Tue/Wed that falls OUTSIDE "
            "the iteration-ending week. The DB save will resume automatically "
            "in the week that the iteration ends."
        )
        logger.info("=" * 60)
        return output_file  # exit main() without touching the DB
    ...
    if save_to_db in ['yes', 'y']:
        ...
        logger.info("="*60)
        # Ensure we have GREENPLUM_SCHEMA available (fetch PROPS where needed)
        ensure_greenplum_schema()

        # Determine Greenplum password/credentials:
        # - If caller provided pg_password, use it
        # - Else when running in Airflow, try to resolve credentials from Airflow Connection
        # - Otherwise prompt interactively (local developer)
        if pg_password:
            GREENPLUM_PASSWORD = pg_password
        else:
            creds = get_greenplum_credentials()
            if creds and creds.get('password'):
                # prefer Airflow-provided connection details
                try:
                    # assign to module-level GREENPLUM_* so save_to_greenplum uses correct host/db/user
                    global GREENPLUM_HOST, GREENPLUM_PORT, GREENPLUM_DB, GREENPLUM_USER
                    GREENPLUM_HOST = creds.get('host', GREENPLUM_HOST)
                    GREENPLUM_PORT = creds.get('port', GREENPLUM_PORT)
                    GREENPLUM_DB = creds.get('db', GREENPLUM_DB)
                    GREENPLUM_USER = creds.get('user', GREENPLUM_USER)
                except Exception:
                    pass
                GREENPLUM_PASSWORD = creds.get('password')
            else:
                GREENPLUM_PASSWORD = getpass.getpass("Enter Greenplum database password: ")
        
        # Save Issues Report and get batch ID
        logger.info(f"\n📊 Saving Issues_Report to {GREENPLUM_SCHEMA}.{OUTPUT_TABLE1}...")
        success1, batch_id = save_to_greenplum(df_final, OUTPUT_TABLE1, GREENPLUM_SCHEMA, GREENPLUM_PASSWORD)
        
        # Save ODM Release Details using the same batch ID
        if not df_odm_details.empty:
            logger.info(f"\n📊 Saving odm_release_details to {GREENPLUM_SCHEMA}.{OUTPUT_TABLE2}...")
            success2, _ = save_to_greenplum(df_odm_details, OUTPUT_TABLE2, GREENPLUM_SCHEMA, GREENPLUM_PASSWORD, batch_id=batch_id)
        else:
...
def generate_release_reports(run_tag: Optional[str] = None,
                           iteration: Optional[str] = None,
                           private_token: Optional[str] = None,
                           postgres_conn_id: Optional[str] = None,
                           **kwargs):
    """Wrapper suitable for Airflow PythonOperator.

    - When executing in Airflow this will read Airflow Variables if
      values are not provided. It will run non-interactively (no
      prompts).
    - When called locally the caller can pass private_token/iteration
      to preserve existing behavior.
    """
    if private_token is None and is_running_in_airflow():
        private_token = get_private_token(allow_prompt=False)

    if iteration is None and is_running_in_airflow():
        iteration = 'current'

    # We intentionally run non-interactively when invoked via Airflow
    return main(private_token=private_token, iteration=iteration, pg_password=None, run_tag=run_tag, allow_prompt=False)
