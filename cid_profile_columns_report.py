...
4. Load the XLSX content into the Greenplum table
   `sandbox_prj_smart_insights.cid_columns_metadata_auto_refresh`, replacing any
   existing data.
...
TARGET_TABLE = "cid_columns_metadata_auto_refresh"
...
def main() -> None:
    ...
    apply_profile_overrides(profile_map)
    ...
    md5_df = apply_dataframe_overrides(md5_df)
...
def apply_profile_overrides(profile_map: Dict[str, ProfileInfo]) -> None:
    for target_type, override in PROFILE_OVERRIDES.items():
        key = target_type.lower()
        info = profile_map.get(key)
        if info is None:
            info = ProfileInfo(target_type=target_type)
            profile_map[key] = info
        profile_table_value = override.get("profile_table")
        joining_key_value = override.get("joining_key")
        if profile_table_value:
            info.profile_table = profile_table_value
        if joining_key_value:
            info.joining_key = joining_key_value


def apply_dataframe_overrides(df: pd.DataFrame) -> pd.DataFrame:
    if df.empty:
        return df
    lower_target_type = df["target_type"].fillna("").str.lower()
    for target_type, override in PROFILE_OVERRIDES.items():
        mask = lower_target_type == target_type
        if not mask.any():
            continue
        for column in ("profile_table", "joining_key"):
            override_value = override.get(column)
            if override_value is not None:
                df.loc[mask, column] = override_value
    return df
