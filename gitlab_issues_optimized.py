...
Faster processing with parallel API calls and bulk database operations
"""
...
class GitLabIssuesFetcher:
    ...
    def fetch_all_issues(self, project_id: str, project_name: str) -> List[Dict[str, Any]]:
        ...
        # Step 1: Fetch all issues (fast, no links)
        while True:
            params = {
                'per_page': per_page,
                'page': page,
        ...
        # Step 2: Fetch all links in parallel (MUCH FASTER)
        if all_issues:
    ...
    def extract_issue_data(self, issues: List[Dict[str, Any]], project_identifier: str) -> pd.DataFrame:
        """Extract comprehensive issue data with consolidated links"""
        extracted_data = []
...
class GreenplumLoader:
    ...
    def insert_data(self, table_name: str, df: pd.DataFrame, batch_size: int = 1000):
        ...
        try:
            ...
            # Use execute_batch for faster bulk insert
...
def create_excel_with_sheets(ikg_df: pd.DataFrame, swat_df: pd.DataFrame, filename: str):
    """Create Excel file with two worksheets - OPTIMIZED"""
    print("\nCreating Excel file...")
    
    wb = Workbook(write_only=False)  # Use regular mode for better compatibility
    wb.remove(wb.active)
...
def main():
    ...
    print("=" * 80)
    print("GitLab Issues Fetcher - OPTIMIZED VERSION")
    print("Faster processing with parallel API calls and bulk operations")
    print("=" * 80)
    ...
    # Step 2: Fetch IKG issues (with parallel link fetching)
    ...
    # Step 3: Fetch SWAT issues (with parallel link fetching)
    ...
    # Step 6: Create tables (parallel not needed, already fast)
    ...
    # Step 7: Insert data (using bulk operations)
    print("\nInserting data into main tables (bulk insert)...")
    loader.insert_data(OUTPUT_TABLE1, ikg_df, batch_size=1000)
    loader.insert_data(OUTPUT_TABLE2, swat_df, batch_size=1000)
    
    print("\nInserting data into archive tables (bulk insert)...")
    loader.insert_data(ARCHIVE_TABLE1, ikg_df, batch_size=1000)
    loader.insert_data(ARCHIVE_TABLE2, swat_df, batch_size=1000)
